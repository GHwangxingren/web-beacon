 ...
 * @LastEditors: 李大玄
 * @LastEditTime: 2022-06-30 19:37:25
 * @FilePath: /el-packaging-use/node_modules/web_beacon/src/service/request.js
 */
...
export function serveApi(instance, name, ApiMap, apiBaseUrl) {
  let service = ApiMap[name];
  // service = Object.assign({}, service, {
  //   getMenuList: window.appConfig.MenuListApi
  // })
  console.log(service);
  ...
  instance.interceptors.request.use(
    (config) => {
      if (window.store.getters.token) {
        config.headers["Authorization"] = window.store.state.user.token;
      }
      return config;
    },
    (err) => {
